...
export interface SavingsState {
  ...
  depositToSaving: (savingId: number, amount: number, pin?: string) => Promise<void>;
  withdrawFromSaving: (savingId: number, amount: number, pin: string) => Promise<void>;
}

// Create the store
export const useSavingsStore: StateCreator<SavingsState> = (set, get) => ({
  // Initial state
  plans: [],
  savings: [],
  portfolio: null,
  ...
  updateSaving: async (id, data) => {
    ...
    try {
      ...
      set((state) => ({ 
        ...
        activeSaving: state.activeSaving?.id === id ? updatedSaving : state.activeSaving,
        isLoading: false
      }));
    } catch (error) {
      set({ isLoading: false });
  ...
  deleteSaving: async (id) => {
    ...
    try {
      ...
      set((state) => ({ 
        ...
        activeSaving: state.activeSaving?.id === id ? null : state.activeSaving,
        isLoading: false
      }));
    } catch (error) {
      set({ isLoading: false });
  ...
  depositToSaving: async (savingId, amount, pin) => {
    set({ isLoading: true });
    try {
      const updatedSaving = await API.savings.deposit(savingId, amount, pin);
      set((state) => ({ 
        savings: state.savings.map(saving => 
          saving.id === savingId ? updatedSaving : saving
        ),
        activeSaving: state.activeSaving?.id === savingId ? updatedSaving : state.activeSaving,
        isLoading: false
      }));
  ...
  withdrawFromSaving: async (savingId, amount, pin) => {
    set({ isLoading: true });
    try {
      const updatedSaving = await API.savings.withdraw(savingId, amount, pin);
      set((state) => ({ 
        savings: state.savings.map(saving => 
          saving.id === savingId ? updatedSaving : saving
        ),
