...
const AuthAPI = {
   ...
   */
  getSocialLoginUrl: (provider: 'google' | 'github' | 'facebook'): Promise<SocialLoginResponse> => {
   ...
   */
  socialLoginCallback: (provider: 'google' | 'github' | 'facebook', code: string): Promise<AuthResponse> => {
    return Http.get(`/login/${provider}/callback?code=${encodeURIComponent(code)}`);
  },
