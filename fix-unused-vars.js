...
try {
  ...
  results.forEach((result) => {
    ...
    try {
      ...
      console.log(
        `Processing ${filePath} with ${unusedVars.length} unused variables`,
      );
      ...
      unusedVars.forEach((unusedVar) => {
        ...
        let match = unusedVar.message.match(
        ...
        console.log(
          `  • Fixing ${unusedVar.line}:${unusedVar.column} - ${varName} → _${varName}`,
        );
        ...
        const newLine =
          line.substring(0, linePos) + '_' + line.substring(linePos);
